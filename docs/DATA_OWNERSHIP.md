...
Import safety limits are deliberately above normal personal-library exports while keeping hostile inputs bounded: JSON and standalone CSV files may be up to 25 MiB; TV Time ZIPs retain the 100 MiB compressed, 300 MiB expanded, and 75 MiB per-entry limits. An import may contain up to 250,000 logical CSV records, 2,000,000 CSV values, 128 CSV fields per record, 250,000 elements in any decoded JSON collection, and 1,000,000 decoded collection elements in aggregate. TV Time imports additionally retain at most 10,000 unique media entities and 250,000 semantic list memberships before catalog resolution. Ordinary text fields, including list names, are capped at 1 MiB; only TV Time's aggregate `lists-prod-lists.csv` `objects` membership field may use up to 8 MiB. ZIPs may contain up to 1,024 total entries, and duplicate recognized TV Time paths are rejected using locale-stable Unicode case folding and normalization. Imported auto-loaded artwork is retained only from the HTTPS image hosts used by the supported TMDB and TVmaze catalog boundaries, with query strings and fragments removed; Gravatar avatars retain only bounded size and rating options. User-initiated trailer and catalog links are separately host-allowlisted and have fragments removed. Older persisted snapshots are scrubbed through the same boundary before hydration and rewritten when cleanup is required. Private tracking, diary, rating, note, watch-history, and list state is unaffected by that normalization.
